...
async function xFetch<T>(
  endpoint: string,
  options: XApiOptions & RequestInit
...
export async function exchangeToken(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { error: err?.error_description || "トークン交換に失敗しました" };
    }

    const data = await res.json();
    return { data };
  } catch (err) {
    return { error: err instanceof Error ? err.message : "トークン交換エラー" };
  }
}
...
export async function refreshAccessToken(
  ...
  try {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      return { error: err?.error_description || "トークン更新に失敗しました" };
    }

    const data = await res.json();
    return { data };
  } catch (err) {
    return { error: err instanceof Error ? err.message : "トークン更新エラー" };
  }
}
...
export async function getMe(bearerToken: string): Promise<XApiResponse<XUser>> {
  const result = await xFetch<{ data: XUser }>("/users/me?user.fields=public_metrics", {
    bearerToken,
  });
...
export async function postTweet(
...
): Promise<XApiResponse<PostTweetResult>> {
  ...
  const result = await xFetch<{ data: PostTweetResult }>("/tweets", {
    bearerToken,
    method: "POST",
...
export async function searchRecentTweets(
  ...
  const query = new URLSearchParams({
    ...
    sort_order: params.sort_order || "relevancy",
  });

  const result = await xFetch<{ data: SearchTweet[] }>(
    `/tweets/search/recent?${query.toString()}`,
    { bearerToken }
  );
  if (result.error) return { error: result.error };
  return { data: result.data?.data || [] };
...
export async function getUserTweets(
  ...
  const result = await xFetch<{ data: SearchTweet[] }>(
    `/users/${userId}/tweets?${query.toString()}`,
    { bearerToken }
  );
  if (result.error) return { error: result.error };
  return { data: result.data?.data || [] };
...
// ===== トレンド（日本） =====
export async function getJapanTrends(
  bearerToken: string
): Promise<XApiResponse<{ name: string; tweet_volume: number | null }[]>> {
  // X API v2ではトレンドAPIが限定的なため、v1.1のエンドポイントを使用
  try {
    ...
    if (!res.ok) {
      return { error: "トレンド取得に失敗しました" };
    }
    const data = await res.json();
    const trends = data[0]?.trends || [];
    return { data: trends.map((t: { name: string; tweet_volume: number | null }) => ({
      name: t.name,
      tweet_volume: t.tweet_volume,
    })) };
  } catch {
    return { error: "トレンドAPIエラー" };
  }
}
