...
③ 離脱阻止・放置の危険性：「どうかこの画面を閉じないでください」「途中で離れると、せっかく整いはじめた波動の流れが止まってしまいます」「最後まで受け取ることで初めて天使様のお力が届きます」
④ 社会的証明：成就・好転の喜びの声を本人の一人称セリフで（又聞き口調の混在NG）。元ネタの数値はコピーせず独自に
⑤ 軽CTA（高評価・コメント）：「その前に、高評価とチャンネル登録をしてくださいね」「これは天使様のお力を受け取りますという合図になります」。コメントは短い一言を指定（例：「受け取ります」）。コメントCTAはこの1回のみ

## 【中盤】ヒーリング音楽パート（2:00〜10:00）★ヒーリングの核
⑥ 呼吸誘導：「鼻からゆっくり吸って、ゆっくり吐き出してみてください」「吐く息と一緒に、抱えてきた不安を手放していきましょう」
⑦ 身体感覚の先取り：「手のひらが、じんわり温かくなっていませんか」。部位は手のひら・胸・頭を使い回し「天使様のお力が届いたサイン」と意味づけ
...
- 無料鑑定で視る対象も恋愛に合わせる（彼の本音の核心は出し切らない＝有料領域。波動と詳しい状況を視て守護天使様のメッセージを届けるまで）`,
    ...
    targetWordCount: 5000, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "選民フック→ヒーリング音楽パート→放置リスク→無料鑑定。恋愛は否定しない・安心ベース。本音は有料領域" },
...
- 無料鑑定で視る対象も金運に合わせる（お金の流れが変わる時期・受け取りを止めている原因の手前まで＝メッセージ提供。具体の本音や波動調整は有料領域）`,
    ...
    targetWordCount: 5000, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "選民フック→ヒーリング音楽パート→放置リスク→無料鑑定。金運フォーカス。恋愛の顧客心理は混同しない" },
...
- 「だから何もしなくていい」で終わらせず、「力の抜き方を一人で掴むのは難しい→だから守護天使様の声が必要」へ無料鑑定導線を接続`,
    ...
    targetWordCount: 5000, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "選民フック→ヒーリング音楽パート→放置リスク→無料鑑定。総合運はアリサ流の引き寄せ（頑張らない・ありのまま）が軸" },
...
- トーンの鉄則（恋愛・必須）：
  ...
  ・「彼の今の気持ち」に本編で触れて関心を引く（ただし本音の核心は無料鑑定では出し切らない＝有料領域）`,
    ...
    targetWordCount: 5200, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定＋最後のカード予告", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "ローラン式5枚（祝福→五感具体化→常識破壊＋仮想敵批判→強さ＋方向性教育→もう来てる）。恋愛は否定しない・安心ベース。本音は有料領域" },
...
- 終盤の無料鑑定で視る対象も金運に合わせる（お金の流れが変わる時期・受け取りを止めている原因の手前まで＝メッセージ提供。具体の本音や波動調整は有料領域）`,
    ...
    targetWordCount: 5200, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定＋最後のカード予告", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "ローラン式5枚。金運フォーカス。恋愛の顧客心理は混同しない" },
...
- 「だから何もしなくていい」で終わらせず、4枚目で「力の抜き方を一人で掴むのは難しい→だから守護天使様の声が必要」へ接続`,
    ...
    targetWordCount: 5200, hookPattern: "選民宣言（おめでとう/選ばれた）＋理想の未来の肯定＋最後のカード予告", ctaPattern: "軽CTAは序盤1回（コメント一言＋高評価＋登録）／重CTAは無料霊視タロット鑑定の一本", notes: "ローラン式5枚。総合運はアリサ流の引き寄せ（頑張らない・ありのまま）が軸" },
];
