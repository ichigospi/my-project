...
const PRESET_DEFAULTS_VERSION = 8;
...
## 基本構成
① 冒頭（0:00〜2:30目安）: 次の5要素を必ずこの順で入れる
...
⑤ 終盤のCTA: 次の順で必ず入れる
  ...
  (2) チャンネル登録を「大開運方法」として勧める:
      「波動は伝染する。運気の良い人・場と繋がっておくことが開運の近道」という教えを土台に、
      「私（このチャンネル）と繋がっておくことが、今すぐにできる大開運方法」としてチャンネル登録を勧める
      ※言い回しは動画のテーマに合わせて必ずチューニングする
      （例: 金運テーマなら「金運の高い波動に触れ続けることが、お金の流れを変える一番の近道」等）
      ※「ただ動画が見つけやすくなる」という機能的な理由ではなく、開運行動そのものとして意味付けする
  (3) その上で重CTA（無料鑑定）へ接続する:「その上で、もし〜」の形で、個別の運気・向いている開運行動を視る無料鑑定へ自然に繋ぐ
