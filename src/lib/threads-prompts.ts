...
【数値の読み取り】各投稿から数値を正確に読み取ってください。
- 【表示回数（インプレッション/views/閲覧数）】を必ず読み取る（本人は見えるため信頼できる）。
- 表示回数が読めない投稿だけ、いいね数で代用（views=0）。
- 「1.2万」「12,000」「1.2K」等は 12000 の数値に変換する。
- ツリー投稿は【親（先頭画像）】の表示回数・いいね等を採用。

【評価軸・最重要の前提】表示回数の絶対値だけで投稿の優劣を決めてはいけない。
- 伸びやすさ（リーチ）は形式で桁が違う: 長文ツリー ＞ 短文 ＞ 教育メイン投稿。形式をまたいで表示回数を単純比較しない。必ず【同じ形式・同じ目的の中で相対比較】する。
- 各投稿には異なる価値がある。目的で捉えること:
  - リーチ型（長文ツリー/短文）= 表示・拡散・世界観の刷り込み・ドーパミン的な欲求喚起。
...
出力形式（JSONのみを出力。説明文やコードフェンス外の文章は不要）:
{
  ...
  "readPosts": [
    { "label": "投稿1", "type": "推定の形式・目的（tree=長文ツリー / short=短文 / education=教育メイン / other）", "views": 表示回数の数値（読めなければ0）, "likes": 数値, "replies": 数値, "reposts": 数値, "excerpt": "本文冒頭20字程度" }
  ],
  "growingTraits": ["リーチ or 教育の観点で効いている具体的特徴（1項目1文。どの形式で効くかも触れる）", "..."],
  "notGrowingTraits": ["弱い/もったいない具体的特徴（1項目1文）", "..."],
  "logicSuggestions": ["投稿ロジックに追記すると良い具体的な指針（命令形・そのまま使える粒度）を3〜6個。「最大インプ×最大教育」を両立させる指針を優先", "..."],
  "insights": {
    "hooks": ["伸びてるフックの型（短く。例: 数字を冒頭に置く／問いかけで始める）", "..."],
    "strongWords": ["刺さってる強ワード・言い回し（単語やフレーズ）", "..."],
    "growthPatterns": ["伸びやすい傾向（形式ごとに。例: ツリーは冒頭の引きが命／教育は体験談が刺さる）", "..."],
    "toneNotes": ["効いている口調・文体の特徴", "..."],
    "logicNotes": ["繰り返し効いている勝ち筋ロジック（リーチ型・教育型それぞれ）", "..."]
  }
}

注意:
- readPosts は投稿ごとに1件（渡された「▼ 投稿N」の順・件数に合わせる）。読み取った表示回数と推定type を必ず入れる
- 表示回数は【同じ形式内で相対比較】。形式をまたいだ絶対値比較で優劣を決めない
- 低インプの教育投稿を一律「失敗」と書かない。教育・ファン化の価値を評価する
...
export interface ReadPost {
  label: string;
  type?: string; // tree | short | education | other
  views: number;
  likes: number;
  replies: number;
  reposts: number;
