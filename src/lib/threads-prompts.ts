...
- 最重要は【表示回数（インプレッション/views/閲覧数）】。伸び/伸びないは原則この表示回数で判断する（表示回数はこのアカウント本人が見えるため信頼できる）。
- 表示回数が読めない投稿だけ、いいね数で代用（views=0）。
- 「1.2万」「12,000」「1.2K」等は 12000 の数値に変換する。
- ツリー投稿は【親（先頭画像）】の表示回数・いいね等を採用。
- エンゲージ率（いいね÷表示回数）も伸びの質の手がかりにする。

そのうえで、表示回数の高い投稿（伸びた）と低い投稿（伸びなかった）を比較し、伸びる/伸びない要因を投稿の中身（フック・構成・強ワード・テーマ・語り口・長さ・CTA・世界観の出し方・ツリーで続きに引き込む引きなど）に踏み込んで分析してください。
...
出力形式（JSONのみを出力。説明文やコードフェンス外の文章は不要）:
{
  ...
  "readPosts": [
    { "label": "投稿1", "views": 表示回数の数値（読めなければ0）, "likes": 数値, "replies": 数値, "reposts": 数値, "excerpt": "本文冒頭20字程度" }
  ],
...
注意:
- readPosts は投稿ごとに1件（渡された「▼ 投稿N」の順・件数に合わせる）。読み取った表示回数を必ず入れる
- 表示回数を根拠に、高い群と低い群の差分として語る
