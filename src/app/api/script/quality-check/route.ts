...
J. ヒーリングパートの変化設計（ヒーリングパートが無い台本では全項目passにする）
   ヒーリングパートに「変化の過程」がセットで入っているか。
   - 体の変化: 今この瞬間の身体感覚を確認させる問いかけ（例:「胸のあたりがじんわり温かくなってきませんか」）があるか
   - 内側の変化: 体感の意味付け・内的変容の宣言（例:「これであなたの中に億を受け取る器ができました」）があるか
   - 両方あり pass / 片方だけ warn / 両方無し fail。順番が「体の変化→内側の変化」になっていなければ warn
