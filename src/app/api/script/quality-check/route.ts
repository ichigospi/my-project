...
A. 元ネタでハマっていた伸び要素の継承
   ...
   ※正しい継承の形は「同じ効果を持つ訴求・例えを別の言い方にする」こと。
     元ネタの訴求を無視して別の話題・別の切り口に差し替えている場合は、伸び要素の継承漏れとして warn〜fail。
   ※フレーズ単位のパクリは禁止(訴求の中身・構造を引き継ぐのはOK、語句の一字一句コピーは不可)。
...
C. 文字数(5000字以内)
   ...
   - 【不足時の加筆提案（必達）】生成台本が元ネタより300字以上短い場合は、元ネタの書き起こしと生成台本を
     突き合わせ、「元ネタにあって生成台本に無い（または薄い）重要な内容」を具体的にピックアップし、
     不足分を埋める加筆提案を items として最大3件出すこと。各itemの suggestion には
     「どのセクション（直前の文を引用）の後に・元ネタの何の要素を・約何字で加筆するか」を書く。
     優先度は伸び要素（離脱防止の仕掛け・共感の深掘り・体験談・受けた人と受けなかった人の格差描写・実力見せ 等）が上。
     単に「文字数を増やす」という提案は禁止。必ず元ネタの抜け要素の補完として提案する。
   - 【超過時の削減提案（必達）】生成台本が元ネタより300字以上長い場合は、超過の原因になっている
     「削るべき箇所」を具体的にピックアップし、削減提案を items として最大3件出すこと。対象の優先順:
     (1)元ネタ・骨組み・台本ルールに無い余計なオリジナル装飾/前置き (2)同じ内容の重複・言い換えの繰り返し
     (3)役割（欲求喚起/離脱防止/信頼構築/行動喚起）を持たない冗長な文。
     各itemの suggestion には「どの箇所（冒頭の文を引用）を・なぜ削るか・削減で約何字減るか」を書く。
     伸び要素・CTA・チャンネル必須要素を削る提案は禁止（削るのは水増し部分だけ）。
   コメントに実測の文字数を必ず含める。

D. CTAロジックの自然な連結
   高評価/コメント/チャンネル登録/守護画像/LINE鑑定 等のCTAが、ロジックとして自然に繋がっているか。
   - 【予祝コメントの質】コメント誘導の例文が「理想の未来の具体的な出来事」を先に言葉にしたものになっているか
     （例:「100万円入ってきました」「彼から連絡が来ました」）。
     願望形（「〜しますように」「〜したい」）は fail。
     完了形でも具体的な出来事が無いもの（「ありがとうございました」「受け取りました」だけ）は予祝として弱いため warn
   ...
   - 【損失回避のスタイル整合】CTAの損失回避の理由づけが動画スタイルの世界観と一致しているか。
     タロット台本なのに「注がれたエネルギーが漏れる/逃げる」等のヒーリング型理由づけを使っていたら fail
     （タロットは「視えた流れが思わぬ出来事で変わる前に固定する」型が正）
...
F. 同じ内容の文・パートの重複
   ...
   - 語尾の意図的繰り返し(アファメーション部等)は許容
   - 【同一ワードの連呼】ヒーリングパート含め、同じ単語・フレーズを短い間隔で2〜3回繰り返していたら warn（多発なら fail）。語尾の繰り返しは許容だが、単語の連呼は単調さの原因
   - 【読点の打ちすぎ】1文に読点（、）が3つ以上入った文が多発していたら warn（大半の文がそうなら fail）。
     悪い例:「お金のことで、誰にも相談できずに、ひとりで、ずっと抱えてきた人。」→ 無くても通じる読点は削るべき。
     単語・事例の羅列や、間を演出する決めゼリフは例外として許容
   - 【日本語の破綻】動詞と主語・目的語の組み合わせが不自然な文（例:「喜ぶ顔を選んであげたかった」→ 顔は選ぶ対象ではない）、
     前後がつながらない文・指示語の指す先が不明な文・二通りに読める文があれば warn〜fail。該当文をすべて引用し、自然な書き直し案を添えること
...
J. ヒーリングパートの変化設計（ヒーリングパートが無い台本では全項目passにする）
   ヒーリングが「内側が段階的に変わっていく1本の物語」になっているか。
   - 部位の移動（頭→胸→手→腹→足 等）＝段階の進行として、部位ごとに**異なる内側の課題**
     （頭=思い込み/胸=諦めの感情/手=受け取る機能/腹=軸/足=行動の道 等）が解決されているか
   - 各段階に「体感の確認（問いかけ）→ その部位のマイナスが抜ける描写 → 新しい状態への書き換え（途中経過の言い方）」があるか
   - 完成宣言（例: 億を受け取る器ができた／丸ごと届く体になった）が全身が繋がった最後の1回だけか
   - 部位ごとに毎回同じ結論（器ができた等）へ結びつけていたら、段階性の欠如として warn〜fail
   - 体感の描写だけで内側の変化が無い場合は warn、変化の過程自体が無ければ fail

I. タイトル・テーマ整合
   タイトルの具体テーマ（宝くじ/ロト/復縁 等）が台本全体で一貫して主役になっているか。
   - フック・体験談・理想/最悪の未来・ヒーリング/リーディング誘導までテーマに紐づいているか。
     「金運全体」「豊かさ一般」等の上位概念に薄まっていたら warn、テーマの言及がほぼ無ければ fail
   - 口コミ・社会的証明がタイトルと同じテーマの事例だけになっているか。
     別テーマ（昇給・臨時収入等）の事例が混ざっていたら warn、テーマ事例ゼロなら fail
   - タイトルの数字・スケールと本文の具体性が釣り合っているか
...
【出力JSONフォーマット - これ以外何も出力しない】
{
  "categories": [
    ...
    { "name": "I. タイトル・テーマ整合", "passed": ..., "items": [...] },
    { "name": "J. ヒーリングパートの変化設計", "passed": ..., "items": [...] }
  ],
  "comparison": [
    {
...
- 「予祝コメント」の行は【理想の未来の具体的な出来事を、もう起きたこととして書かせているか】を基準に採点する。願望形は △以下、出来事の無い感謝だけ（「ありがとうございました」等）も △以下とする。
...
- 次に重視: A(元ネタ要素継承), D(CTAロジック), G(矛盾), I(タイトル・テーマ整合)
- 中程度: B(オリジナル混入), F(重複)
- 軽め: C(文字数)
...
- suggestion は warn / fail の項目だけ、1文・最大50字。ただしC観点の【不足時の加筆提案】【超過時の削減提案】のsuggestionのみ最大100字まで許容（位置と内容を具体的に書くため）。
