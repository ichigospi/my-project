...
B. 元ネタとの比較: 訴求ポイント / フック / 伸びた要因 / 「理想の未来」「最悪の未来」が
   ...
   ※特に「具体的金額(3億円など)」「具体的シチュエーション(口座に振込通知)」が
     抽象化されてないか厳しくチェック
   ※【フレーズ単位のパクリチェック】元ネタ台本に出てくる以下の表現がそのまま流用されていないか厳しく見る:
     ・具体的金額・宝くじ種類・時期・売り場の特徴のセット(例:「翌月にジャンボで1等5億円」「言われた日に言われた売り場」)
     ・3つの疑問形の中身(例:「いつ買えばいいか／どこで買えばいいか／いくら当たるか」)
     ・比喩・対比表現(例:「天と地ほどの差」「水と油」)
     ・体験談のセリフ・締め文・キャッチフレーズ
     1つでも元ネタとほぼ同一の表現があれば warn、複数あれば fail。
     構造・役割を引き継ぐのは可、語句・固有要素・数値の流用は不可
...
D. 構成密度: 台本ルールで定義された構成 (フック→展開→クライマックス→CTA等) を遵守してるか
   ...
   ※内部整合性: パート間で主張が矛盾していないか（例:「受け取るサイン」が複数あって食い違う等）。
     矛盾があれば fail。同じCTA(特にチャンネル登録)を何度も繰り返していないか（くどい重複は warn〜fail）
   ※損失回避×インセンティブの連結: 「放置の危険性(損失)」を語る箇所が、
     具体的なアクション(LINE登録等)と、そのアクションが何を達成するか までワンセットで繋がっているか。
     損失だけ／アクションだけで終わっていれば warn〜fail
   ※CTA間の理由づけ矛盾チェック（最重要）: 複数のCTAで「同じ課題を解決する」と言っていないか。
     例: 序盤コメントCTAで「エネルギーを固定するために『受け取ります』と書いて」と誘導しておきながら、
     終盤の守護画像CTAやLINE鑑定CTAで「エネルギーが定まらない／波動が安定しない」と再び言うのは矛盾。
     視聴者に「前のCTAは無駄だったのか」と感じさせる重大なロジック破綻。発見したら fail
   ※守護画像→霊視鑑定の一本線チェック: 重CTAで守護画像とLINE霊視鑑定の両方を出している場合、
     ①ベネフィット獲得宣言 → ②損失リスク → ③守護画像CTA(エネルギー固定) → ④不完全感(具体行動が視えない)
     → ⑤霊視鑑定CTA(行動指針) → ⑥比較強調 の順で一本の線になっているか。
     守護画像と霊視鑑定の役割が被って(両方とも"エネルギー定着"のためになって)いないか。
     役割が被っていれば warn〜fail。順番が乱れていれば warn
   ※体験談・社会的証明の話者整合性チェック: 「」で囲まれた体験談の中身が、必ずお客様本人の一人称セリフ(直接話法)で
     統一されているか。「○○という方もいらっしゃいます」「○○されたご報告があります」のようなナレーター視点(又聞き口調)
     を「」の中に入れていないか。本人の声と又聞き口調が混在していたら fail
     (悪い例:「再生した翌日に大きな臨時収入が届きました」と「宝くじで1等2億円が当選したという方もいらっしゃいます」
     が「」で並列になっている)
