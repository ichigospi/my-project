...
【評価観点 / 4カテゴリ】
A. 構成ルール遵守 … 下記【台本ルール】の構成・口調・書式・CTAルールに沿っているか（このパートの範囲で）。予祝コメントの例文があれば「理想の未来の具体的な出来事」を先に言葉にしたものか（例:「100万円入ってきました」「彼から連絡が来ました」。願望形はfail、出来事の無い「ありがとうございました」だけはwarn）。CTAの損失回避の理由づけがスタイルの世界観と一致しているか（タロットで「注がれたエネルギーが漏れる」型はヒーリング混入としてfail。タロットは「視えた流れが思わぬ出来事で変わる前に」型が正）。ヒーリングパートがあれば「変化の過程」が段階的か: 部位の移動ごとに異なる内側の課題（思い込み/諦め/受け取る機能/軸/行動の道 等）が解決され、各段階に体感確認→マイナスが抜ける→書き換え（途中経過の言い方）があるか。完成宣言（器ができた等）が最後の1回だけか（部位ごとに毎回同じ結論へ結びつけていたら段階性の欠如としてwarn〜fail。体感だけで意味付けが無い場合もwarn）。
B. 元台本からのズレ … 参考元の該当範囲の構成・訴求・具体性を継承し上位互換になっているか。正しい継承は「同じ効果を持つ訴求・例えを別の言い方にする」こと（別の話題への差し替えは継承漏れとして減点）。ヒーリングパートがあれば誘導の流れ・情景展開が元ネタをトレースしているか。フレーズ単位のパクリ（金額・比喩・疑問形・体験談セリフの一字一句コピー）が無いか。タイトルの具体テーマ（宝くじ等）にこのパートの内容・口コミ・事例が紐づいているか（金運一般等に薄まっていないか／別テーマの事例が混ざっていないか）。
C. 表現品質 … 不自然な日本語・奇妙な体感表現（「お腹が締め付けられる」等）・共感を得にくい訴求・語尾の単調な連続・冗長な水増しが無いか。動詞と主語・目的語の組み合わせが破綻した文（例:「喜ぶ顔を選んであげたかった」→ 顔は選ぶ対象ではない）や、前後がつながらない文・指示語の指す先が不明な文があればwarn〜fail（該当文を引用し書き直し案を添える）。同じ単語・フレーズを短い間隔で2〜3回連呼していないか（ヒーリングパートも対象。語尾の繰り返しは許容だが単語の連呼はNG）。読点（、）の打ちすぎがないか: 1文に読点3つ以上の文が多発していたらwarn（悪い例:「お金のことで、誰にも相談できずに、ひとりで、ずっと抱えてきた人。」。単語の羅列・間を演出する決めゼリフは例外）。時期訴求（「◯月」「この夏」等の予言）が下記の現在日時を基点に未来になっているか（過ぎた月を未来として語っていたらfail）。
...
【現在日時（日本時間）】${new Date().toLocaleDateString("ja-JP", { timeZone: "Asia/Tokyo", year: "numeric", month: "long", day: "numeric" })}
