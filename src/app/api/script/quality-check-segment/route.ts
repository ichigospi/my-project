...
C. 表現品質 … 不自然な日本語・奇妙な体感表現（「お腹が締め付けられる」等）・共感を得にくい訴求・語尾の単調な連続・冗長な水増しが無いか。同じ単語・フレーズを短い間隔で2〜3回連呼していないか（ヒーリングパートも対象。語尾の繰り返しは許容だが単語の連呼はNG）。時期訴求（「◯月」「この夏」等の予言）が下記の現在日時を基点に未来になっているか（過ぎた月を未来として語っていたらfail）。
