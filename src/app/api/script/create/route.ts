...
- 【ヒーリングパートの中身も元ネタをトレースする（必達）】誘導の流れ・イメージさせる情景・語りかけの展開は、元ネタのヒーリングパートを忠実になぞること。勝手なオリジナル瞑想・独自の誘導手順に置き換えない。その上で、情景・語彙だけをタイトルのテーマ（宝くじ等）と自チャンネルの世界観に合わせて具体化する
- 【ヒーリングパートの変化の過程（必達）】ヒーリングは「内側が段階的に変わっていく1本の物語」として書く。
  ・お力が体を上から下へ流れる移動（頭→胸→手→腹→足 等）＝変化の段階。**部位ごとに解決する内側の課題を変える**こと
    （例: 頭＝「私には当たらない」という思い込み・思考の声を溶かす／胸＝諦め・感情のブロックを抜く／手＝受け取る機能を作り変える／腹＝軸・中心を整える／足＝行動と導きの道を開通させる。部位が象徴する意味に対応させる）
  ・各部位のミニ構造: (1)今ある嫌な内面の言語化・共感（必要に応じて）→ (2)お力がその部位へ移動 → (3)体感の確認（「温かいもの、感じませんか」等の問いかけ）→ (4)その部位に溜まっていた具体的なマイナスが抜ける/溶ける/剥がれる描写 → (5)新しい状態への書き換え。ただし途中の部位では「〜の合図や」「整い始めた」のような**途中経過の言い方**にとどめる
  ・マイナスは視聴者が心当たりのある内面の言葉で書く（「私には当たらない」という頭の声、当選話で胸が苦しくなる感覚 等）
  ・**完成宣言（例:「これで億を受け取る器ができた」「削られずに丸ごと届く体になった」）は、全部位が終わって全身が繋がった時に1回だけ**。部位ごとに毎回同じ結論（器ができた等）に結びつけるのは禁止（変化が進んでいる感じが消えるため）
  ・完成後、流れを固定する簡単な日常アクションを1つ授ける（例: 朝、太陽に向かって「受け取ります」とひと言）
...
※【トレースの基本イメージ（最重要）】
  トレースとは「**別の話に差し替える**」ことではなく、「**同じ効果・同じ役割を持つ訴求や例えを、別の言い方で言い直す**」こと。
  元ネタの各訴求について、狙っている感情・訴求の中身・例えの構図はそのまま維持し、**表面の語句だけを自チャンネルの言葉に置き換える**。
  訴求の中身ごと別の話題に入れ替えると、元ネタがハマった理由（＝その訴求そのもの）が失われるため禁止。
※【フレーズ単位のパクリ厳禁（必達）】元ネタ分析・元ネタ台本に書かれた以下を**一字一句そのまま流用するのは禁止**:
  - 具体的な金額・宝くじ種類・時期・売り場の特徴（例: 「翌月にジャンボで1等5億円」「言われた日に言われた売り場」）
  - 疑問形・比喩・対比表現の言い回し（例: 「天と地ほどの差」「水と油」）
  - 体験談のセリフ・ナレーション・言い回し
  - 固有のフック文・締め文・キャッチフレーズ
  引き継ぐのは **訴求の中身・構造・役割・具体性のレベル**。言い回し・固有名詞・数値設定だけを自チャンネル独自に作り直すこと。
  ・良い例: 元ネタ「翌月にジャンボで1等5億円」→ 自前「3週間後にミニロトで780万円」（同じ訴求＝短期間での高額当選体験。数値と語句だけ変更）
  ・良い例: 元ネタ「いつ買えばいいか／どこで買えばいいか／いくら当たるか」→ 自前「買うべきタイミング／選ぶべき売り場／舞い込む金額」（**同じ3つの疑問**を別の言い回しにする。疑問の中身を別の話題に差し替えない）
  ・良い例: 元ネタ「天と地ほどの差」→ 自前「朝日と夕暮れほどの差」「月とすっぽんほどの差」（同じ対比効果を別の言葉で）
  ・悪い例: 元ネタの数値・宝くじ種類・売り場特徴・疑問形・比喩を一字一句そのまま使う → 一致率25%以上で失格
  ・悪い例: 元ネタの訴求を無視して、別の話題・別の切り口に置き換える → 伸び要素の継承漏れで失格
...
【テーマの一貫（必達）】
- タイトルの具体テーマ（例:「宝くじ」「ロト」「臨時収入」「復縁」等）を、台本全体で一貫して主役にすること。
  タイトルが「宝くじ」なら、フック・体験談・理想の未来・最悪の未来・ヒーリング/リーディングの誘導・口コミまで、すべて宝くじに紐づけて語る。
- 「金運全体」「豊かさ一般」のような上位概念の話に薄めない。一般論に逃げた台本は失格。
- 口コミ・社会的証明もテーマ一致（必達）: タイトルが宝くじなら、引用する口コミ・事例は宝くじ当選のものだけにする。
  昇給・臨時収入・借金完済など別テーマの事例を混ぜない（テーマが薄まり、タイトルとの約束が壊れるため）。
...
- 同じ語尾の連続使用の制限がルールにある場合でも、ヒーリングパート・アファメーションパートには適用しない（語尾の連続は演出として許容）。それ以外のパートでは制限を守る
- ただし【同一ワードの連呼禁止（ヒーリング含む・必達）】: 語尾は繰り返してよいが、同じ単語・同じフレーズを短い間隔で2〜3回繰り返すのは禁止（例:「金色の光が…金色の光が…金色の光に…」）。2回目以降は言い換え・別の情景・別の語彙で展開し、単調な連呼にしない
...
6. 出力前の自己校閲（必達・最重要）
   台本を出力する前に、書いた全文を頭から読み直し、次の観点で不合格の文をすべて書き直してから出力すること:
   - 文として成立しているか: 動詞と主語・目的語の組み合わせが日本語として自然か
     （悪い例:「喜ぶ顔を選んであげたかった」→ 顔は「選ぶ」対象ではない。正:「喜ぶ顔が見たくて、選んであげたかった」等）
   - 前後がつながっているか: 各文が直前の文を受けて意味が通るか。話が飛んでいないか。指示語（それ・その〜）の指す先が明確か
   - 一読で意味が取れるか: 読み返さないと分からない文・二通りに読める文・分かりにくい言い回しは書き直す
   - 声に出して自然か: これはナレーション原稿。読み上げて引っかかる文はすべて修正する

7. 読点（、）を打ちすぎない（必達）
   - 無くても意味が通じる場所に読点を打たない。1文の中の読点は最大2つまでを原則とする
   - 悪い例:「お金のことで、誰にも相談できずに、ひとりで、ずっと抱えてきた人。」
   - 良い例:「お金のことで誰にも相談できず、ひとりでずっと抱えてきた人。」
   - 例外: 単語・事例を羅列する箇所（「宝くじ、臨時収入、昇給」等）や、間を演出として明確に必要とする決めゼリフのみ3つ以上を許容
   - 細切れの読点はナレーションのリズムを壊し、素人っぽく聞こえる。迷ったら読点を削る方を選ぶ
...
B. 軽CTAの3例文
   ...
   - 【予祝＝理想の未来の出来事を先に言葉にする（必達）】
     予祝とは「完了形で感謝する」ことではなく、**理想の未来に起きる具体的な出来事を、もう起きたこととして先に言葉にする**こと。
     良い例:「100万円入ってきました」「彼から連絡が来ました」「宝くじ、1等当たりました」
     弱い例（完了形だが出来事が無い→予祝にならない）:「ありがとうございました」「受け取りました」「感謝します」だけのもの
     悪い例（願望形は禁止）:「当たりますように」「連絡が来てほしい」「受け取りたいです」
     例文3つは、それぞれ**別の具体的な出来事**（金額・相手・状況が入ったもの）にし、タイトルのテーマに紐づけること。

C. 軽CTA直後の社会的証明
   - 「以前○○と書いた方が、翌月××になりました」型で具体時系列＋具体金額/結果を1件
   - 「多くの方が」のような抽象表現は禁止
   - お客様の声を「」で引用する場合は必ず本人の一人称セリフで書く。「○○という方もいらっしゃいます」型のナレーター視点を「」に入れるのは絶対NG
   - 【テーマ一致】事例はタイトルのテーマと同じ種類にする（宝くじテーマなら宝くじ当選の事例のみ。別テーマの成果を混ぜない）
...
D. 軽→重の橋渡し（サンクコスト型・損失回避×インセンティブの連結）★最重要
   ...
   - 【損失回避の理由づけはスタイルの世界観に合わせる（必達）】
     ・ヒーリング系: 「注がれたお力・エネルギーが日常の不安やストレスで漏れていく」型の理由づけを使ってよい
     ・タロット系: エネルギー注入は行っていないため「注がれたエネルギーが漏れる/逃げる」型は禁止（ヒーリングの世界観の混入）。
       代わりに「せっかく視えた流れが、思わぬ出来事やアクシデント・迷いをきっかけに変わってしまう可能性がある。
       流れが変わる前に固定する」型など、リーディングで視えたもの（流れ・カードが示した未来）を基点にした理由づけにする
...
F. 重CTA直後の社会的証明（スケールアップ）
   ...
   - お客様の声を「」で引用する場合は必ず本人の一人称セリフで書く。「○○という方もいらっしゃいます」型のナレーター視点を「」に入れるのは絶対NG
   - 【テーマ一致】ここでもタイトルのテーマと同じ種類の事例だけを使う（宝くじテーマなら宝くじ当選のみ）
...
理由づけを必ず下記の1本の線として接続すること。各ブロックの順番・役割を入れ替えない。
  ...
  ※タロット系では①〜③の「エネルギー」を「視えた流れ／カードが示した未来」に読み替えること（エネルギー注入・漏れの表現は使わない）
