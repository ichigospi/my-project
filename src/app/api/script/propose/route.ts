...
2. 第1CTA（軽め・心理ハードル低）+ 例文3パターン + 社会的証明
   ...
   - 例文は【予祝＝理想の未来の具体的な出来事を先に言葉にする】で設計する（例:「100万円入ってきました」「彼から連絡が来ました」。願望形は禁止、出来事の無い「ありがとうございました」だけも予祝にならないと明記）
...
3. 軽→重への橋渡し（サンクコスト型ペナルティ訴求）★最重要
   ...
   - 理由づけはスタイルの世界観に合わせて設計する: ヒーリング系は「注がれたエネルギーが漏れる」型OK。
     タロット系はエネルギー注入をしていないため「せっかく視えた流れが、思わぬ出来事やアクシデントで変わってしまう前に固定する」型にする（エネルギー漏れ型はヒーリング専用で禁止）
...
- カード名は必ず日本語で言う（例:「星」「太陽」「運命の輪」「カップの2」）。英語・カタカナ読み（ザ・スター／ツー・オブ・カップス等）は禁止。
