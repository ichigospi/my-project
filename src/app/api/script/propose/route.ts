...
- 【ヒーリングパートの変化設計（必達）】ヒーリングパートの設計には「変化の過程」を必ず明記すること。
  ・体の変化: 視聴者に今この瞬間の身体感覚を確認させる問いかけ（例:「だんだん胸のあたりがじんわり温かくなってきませんか？」）
  ・内側の変化: その感覚が何を意味するかの内的変容の宣言（例:「これであなたの中に億を受け取る器ができました」）
  この2つを「体の変化（体感の確認）→ 内側の変化（変容の意味付け）」のセットとして、ヒーリングセクションの設計に必ず書き込む。体感だけ・意味付けだけの片方は不可
