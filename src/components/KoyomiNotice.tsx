// 開運日通知バナー: 全Threadsページ上部に表示。当日・明日・2日後の開運日/満月新月を知らせる。
"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import { luckyForDate, offsetLabel, type LuckyEvent, type UpcomingNotice, upcomingNotices } from "@/lib/threads-koyomi";
...
export default function KoyomiNotice() {
  ...
  if (notices === null) return null; // 計算前（SSRとの不一致回避）
  ...
  if (notices.length > 0) {
    ...
    return (
      <div
        className={`px-4 md:px-6 py-2 border-b flex flex-wrap items-center gap-x-3 gap-y-1.5 ${
          hasStrong
      ...
      >
        ...
        <Link
          href="/threads/calendar"
          className="text-[11px] text-neutral-400 hover:text-neutral-200 whitespace-nowrap underline"
        >
          カレンダー
        </Link>
      </div>
  ...
  // 直近3日に無い場合は「次の開運日」を控えめに表示
  if (next) {
    return (
      <div className="px-4 md:px-6 py-1.5 border-b border-neutral-800 bg-neutral-900/40 flex flex-wrap items-center gap-2">
        ...
        <Link
          href="/threads/calendar"
          className="ml-auto text-[11px] text-neutral-500 hover:text-neutral-300 underline whitespace-nowrap"
        >
          カレンダーを見る
        </Link>
      </div>
