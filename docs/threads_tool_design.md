...
既存の Xポストツール（`/x-post`）と同じ思想・同じ技術スタックの上に、Threads専用モジュール `/threads` として構築する。
...
### 0-2. Claude APIは画像・動画を生成できない

Claude APIはテキスト生成・分析専用。画像/動画は別系統が必要。

- **画像（推奨・Phase 1）**: Threadsで伸びる「テキスト画像」はテンプレート合成で作れる。サーバー内で satori（HTML→SVG）+ sharp でPNG化。API費用ゼロ・爆速・ブランド統一。
- **画像（Phase 3）**: 生成AI画像が必要なら Gemini（Imagen系）/ OpenAI gpt-image-1 を外部呼び出し。
...
| AI | `@anthropic-ai/sdk`（導入済み） | 生成・分析・分類・壁打ちすべてClaude |
...
ThreadsCompetitorPost ←─ importPost(input): 共通インターフェース
        ...
        ├─ ② キーワード検索API（Phase 2）: threads_keyword_search で
        │    ジャンルKWの伸び投稿を定期収集 → 競合ハンドルと突合
        └─ ③ 外部スクレイパー（Phase 3・任意）: Apify等のActorを
             設定画面から接続。日次で競合の新着+数値を自動取り込み
```
...
| **2: API接続** | Threads OAuth、予約自動投稿cron、インサイト自動計測cron、キーワード検索収集、アナリティクス画面 | 「自動入力」「反応計測」が完成 |
| **3: 拡張** | テンプレ画像生成（satori）→AI画像、スライド動画、外部スクレイパー接続、企画×成績のクロス分析 | 運用の完全自動化に接近 |

---

## 10. 要確認事項（実装前にオーナー判断が欲しい点）

1. **Meta開発者アプリ**: Threads API利用にはMetaアプリ登録+権限審査が必要（特にkeyword_search）。アプリを作る/既にあるか。
2. **公開の最終トリガー**: cronによる完全自動公開か、「予約時刻に通知→人がワンクリック公開」か（誤爆リスクの好み）。Phase 2の既定は自動を想定。
3. **競合自動収集にApify等の有料スクレイパーを使うか**（月額数十$）。使わない場合は手動貼り付け+キーワード検索APIで運用。
4. **画像生成**: テンプレ合成（無料・統一感）で十分か、生成AI画像（従量課金）まで必要か。
