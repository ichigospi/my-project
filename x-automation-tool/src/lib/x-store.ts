...
export interface XArticleLocal {
  ...
  title: string;
  content: string;
  sourceType: "from_script" | "trend" | "curated";
...
export function getAiApiKey(): string {
  if (!isBrowser()) return "";
  return localStorage.getItem(KEYS.aiApiKey) || "";
}

export function setAiApiKey(value: string): void {
  if (!isBrowser()) return;
  localStorage.setItem(KEYS.aiApiKey, value);
}
...
export function getXBearerToken(): string {
  if (!isBrowser()) return "";
  return localStorage.getItem(KEYS.bearerToken) || "";
}

export function setXBearerToken(token: string): void {
  if (!isBrowser()) return;
  localStorage.setItem(KEYS.bearerToken, token);
}
...
export function addXPost(post: Omit<XPostLocal, "id" | "createdAt">): XPostLocal {
  ...
  const newPost: XPostLocal = { ...post, id: genId(), createdAt: new Date().toISOString() };
...
export function deleteXPost(id: string): void {
  saveXPosts(getXPosts().filter((p) => p.id !== id));
}
...
export function addXArticle(article: Omit<XArticleLocal, "id" | "createdAt" | "updatedAt">): XArticleLocal {
  ...
  const newArticle: XArticleLocal = { ...article, id: genId(), createdAt: now, updatedAt: now };
...
export function deleteXArticle(id: string): void {
  saveXArticles(getXArticles().filter((a) => a.id !== id));
}
