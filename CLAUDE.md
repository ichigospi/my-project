...
`video_system/` に動画制作システム（台本作成→素材準備/AI生成→テロップ・BGM編集→書き出し→修正）があります。
ユーザーから「動画を作って」「動画を編集して」「テロップを入れて」「BGMを付けて」「ショート動画を作って」等の指示があった場合は、このシステムを使ってください。

詳細なルールは `@video_system/CLAUDE.md` を参照。
