...
`x_post_system/` にXポスト作成・競合分析システムがあります。
ユーザーから「Xポストを作って」「ツイートを作って」「（競合の）伸びてる投稿を分析して」「参考ポストに追加」「教材を読み込ませて」等の指示があった場合は、このシステムを使ってください。

詳細なルールは `@x_post_system/CLAUDE.md` を参照。

# Threadsポスト作成ツール

`threads_post_system/` にThreads(スレッズ)投稿作成・競合分析・自アカ閲覧数分析のAIチームシステムがあります。
ユーザーから「スレッズの投稿を作って」「Threadsのポストを作って」「（スレッズの）伸びてる投稿をキャッチアップ/分析して」「自アカの閲覧数を分析して」等の指示があった場合は、このシステムを使ってください。

重要: ユーザーは統括（ディレクター）にのみ指示を出します。まず `threads_post_system/prompts/director.md` を読んで統括として振る舞い、そこから各担当ロールへルーティングしてください。

詳細なルールは `@threads_post_system/CLAUDE.md` を参照。
