...
```
RUNPOD_API_KEY=...
RUNPOD_ENDPOINT_ID=...              # 通常生成（Lora のみ）
RUNPOD_IPADAPTER_ENDPOINT_ID=...    # 顔固定生成（IP-Adapter Face 同梱イメージ）
RUNPOD_NETWORK_VOLUME_ID=...
CIVITAI_API_KEY=...
ANTHROPIC_API_KEY=...
```

顔参照画像が登録されていて、かつ生成画面で「👤 顔固定」が ON の場合のみ `RUNPOD_IPADAPTER_ENDPOINT_ID` にルーティングされる。未設定でも通常生成は動く。
